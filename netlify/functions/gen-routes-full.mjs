// 一時的: 往復両方のポリラインを正しい経由点で再生成
// 使用後に削除予定
//
// 復路 to-iida 追加経由点:
//   中央広場 → 中央交差点信号 → 銀座4,5丁目信号 → 知久町１丁目
//   飯田病院前 → 元町交差点 → 飯田駅南交差点 → 飯田駅前
// 往路 to-wada 追加経由点:
//   うとどち→学校前 は国道152号沿い (各バス停を経由するためそのままでOK)
//   ※学校前→和田 間は停留所削除済みなので国道152号上の中継点を追加して道沿いに保つ
...
const TO_WADA = [
  ...
  { name: "一本木", loc: "35.48420,137.89680" },
